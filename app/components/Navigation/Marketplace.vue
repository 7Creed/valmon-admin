...
const { getUnreadNotifications, getUnreadMessages } = NotificationsController();
...
const selectCurrency = (id) => {
  const selectedCurrencyData = currencies.value.find(
...
const fetchUnreadMessages = async () => {
  ...
  console.log("Fetching notifications...");
  try {
    const { status, data } = await getUnreadMessages();
    if (status.value === "success") {
      UnreadMessages.value = data.value.data?.unread_messages || 0;
    }
  } catch (error) {
...
<template>
  <nav class="bg-stone-950 fixed w-full z-[1000] hidden lg:block">
      ...
      <div class="relative flex h-20 items-center justify-between">
        ...
        >
          <a href="/" class="flex flex-shrink-0 items-center">
            <img class="w-auto" :src="brandLogo" alt="Valmon Brand Logo" />
          </a>
        </div>
        <!-- Left side  -->
        <div class="flex items-center justify-end flex-1 space-x-4">
          <!-- search input -->
          <div
            class="hidden w-1/4 rounded-lg ring-1 ring-darkGold focus:outline-none focus:ring-2 focus:ring-darkGold bg-stone-900"
          >
            <label
          ...
          <NuxtLink to="chat">
            <button
              ...
              class="relative rounded-full bg-gray-800 p-1 text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
            >
              <img :src="message" alt="heart icon" />
              <span
          ...
          <div class="relative">
            ...
            >
              <div class="flex justify-between items-center mb-3">
                <h3 class="text-sm font-semibold text-black">Notification</h3>
                <a
            ...
            <div class="relative">
              ...
              >
                <span class="flex items-center">
                  ...
                  />
                  <span class="ml-2 block truncate text-brightGold">{{
  ...
  >
    <div class="modal-box text-black w-96">
      ...
      <p class="py-4 text-center">A client just reach out for a job</p>
      <div class="flex items-start gap-6 mb-5 justify-center">
        <!-- avatar -->
        <div class="avatar">
          <div
            class="ring-darkGold ring-offset-base-100 w-16 rounded-full ring ring-offset-2"
          >
            <img
