...
onMounted(() => {
  ...
  const conversation_id = queryParams.get(
    "conversation_id",
    conversations.value
  );

  if (conversation_id) {
    targetConversationId.value = conversation_id;
  }
...
const conservationTab = () => {
  if (conversations.value.length > 0) {
    if (activeTag.value === "all") {
      ...
      activeConversation.value = conversations.value.filter(
...
const fetchConversation = async () => {
  try {
    ...
    if (status.value === "success") {
      ...
      const marketplace = data.value.data.filter(
        (conv) => conv.listing_id != null
      );
...
const sendMessageWithFile = () => {
  ...
  const formData = new FormData();
  formData.append("content", message.value.trim());
  formData.append("type", fileType.value || "text");
  ...
  fileType.value = null;
  textInput.value.readonly = false;
...
const conversationType = computed(() => {
  if (
    selectedConversation.value?.service_id &&
    store.UserAccount.account_type === "employer"
  ) {
    return "employerService";
  }
  if (
    selectedConversation.value?.listing_id &&
    store.UserAccount.account_type === "employer"
  ) {
  ...
  if (
    selectedConversation.value?.service_id &&
    store.UserAccount.account_type === "worker"
  ) {
...
watch(allMessages, (newVal) => {
  ...
  const recipientMessages = newVal.filter(
    (message) => message.type === "negotiation"
  );

  // Get the latest negotiation messages
  const recipientLatest =
    recipientMessages[recipientMessages.length - 1] || null;
  latestOfferNegotiationId.value = recipientLatest
    ? recipientLatest.negotiation_id
    : null;

  // Debugging: Log latest negotiation messages
  console.log(
    "Recipient Last Negotiation Message:",
    recipientLatest,
    recipientMessages,
    recipientLatest ? recipientLatest.negotiation_id : null
  );

  // Update offers based on account type
  latestOffer.value = recipientLatest
    ? recipientLatest.negotiation?.price_offer
    : null;
  ...
  const initiatorMessages = newVal.filter(
    (message) =>
      message.type === "negotiation" && message.user_id == store.UserAccount.id
  );
  initiatorLatestNeg.value =
    initiatorMessages[initiatorMessages.length - 1] || null;
...
watch(negotiationPrice, (newVal) => {
  if (newVal) {
    ...
    const serviceCharge =
      negotiationPrice.value - newVal * serviceChargePercentage;
...
const acceptNegotiation = () => {
  if (
    initiatorLatestNeg.value &&
    initiatorLatestNeg.value.negotiation.accepted
  ) {
...
const _shippingStatus = computed(() => {
  if (conversationType.value === "employerListing") {
    return latestOrder.value.shipping_status === "delivered"
      ? "Product Received"
      : "Awaiting shipment";
  }
  return conversationType.value === "workerListing" &&
    latestOrder.value.shipping_status === "pending"
    ? "Product Delivered"
    : "Completed";
});
...
const sendReviews = () => {
  chatApiWithParams(
    createReview,
    reviewData,
    latestOfferUserId.value || latestJobOfferUserId.value,
    reviewLoader
  );
};
...
const report = () => {
  ...
  reportData.order_id
    ? formdata.append("order_id", reportData.order_id)
...
const MarkCompleted = async () => {
  ...
  const JobStatus =
    store.UserAccount.account_type == "employer" ? "completed" : "in review";
  try {
...
<template>
  ...
  <div v-else class="flex gap-2 xl:gap-10 lg:pt-10 w-full">
    <!-- card 1 -->
    <div
    ...
    >
      <div class="card-body p-4">
        ...
        >
          ...
          <div>
            <div class="flex items-center text-[#101011] gap-2 lg:gap-8 mb-1">
              <span class="text-sm satoshiB"
                >{{ renderConversation(selectedConversation).first_name }}
        ...
        >
          <span class="text-center text-sm lg:base w-full block"
            >Keep all dealings on valmon to ensure safety</span
          >
        </div>

        <!-- This is for mobile -->
        <div v-if="isMobile" class="lg:hidden card bg-base-100 shadow-xl">
          <div class="px-2 py-6">
            <h2 class="card-title mb-1">Service Cost</h2>
            <div class="flex items-center my-3 gap-4">
              ...
              <div class="">
                ...
                <!-- Job Offer Amount -->
                <p v-show="jobStatus === '' && activeTab === 'job'" class="">
                  NGN {{ latestOffer || 0 }}
                </p>
                ...
                <p
                  v-if="
                    selectedConversation?.listing_id && jobStatus != 'Completed'
                  "
                >
                  NGN {{ latestOffer || 0 }}
                </p>
            ...
            <div class="card-actions flex flex-wrap gap-3 items-center">
              ...
              >
                ...
                <span v-else>{{
                  conversationType === "employerService"
                    ? "Accept & Hire"
                    : conversationType === "employerListing"
                    ? "Accept & Buy"
                    : conversationType === "workerListing"
                    ? "Accept offer"
                    : conversationType === "workerService"
                    ? "Accept Proposal"
                    : "Accept "
                }}</span>
              </button>
              ...
              <!-- Services -->
              <div v-if="activeTab == 'job'" class="flex justify-center">
                <button
                  v-if="jobStatus === 'Completed'"
                  :disabled="
                    (latestOrder.status != 'in review' &&
                      store.UserAccount.account_type == 'employer') ||
              ...
              <div v-else class="flex justify-center">
                <button
                  v-if="
                    jobStatus === 'Completed' &&
                    (_shippingStatus === 'Completed' ||
                      _shippingStatus === 'Product Delivered')
                  "
              ...
              <div v-if="jobStatus === 'Completed'" class="flex justify-center">
                <!-- Report -->
                <button
                  v-if="jobStatus === 'Completed'"
                ...
                >
                  <span v-if="activeTab === 'job'">{{
                    store.UserAccount.account_type !== "employer"
                      ? "Report Client"
                      : "Report Worker"
                  }}</span>
                  <span v-if="activeTab === 'marketPlace'">{{
                    store.UserAccount.account_type !== "employer"
                      ? "Report Buyer"
                      : "Report Seller"
                  }}</span>
        ...
        >
          ...
          {{ renderConversation(selectedConversation).first_name }} starts here
        </p>
        ...
        <div class="h-96 overflow-auto">
          ...
          <div
            ...
            class="chat"
            :class="
              mesg.user_id == store.UserAccount.id
                ? 'chat-start flex'
                : 'chat-end'
            "
          >
            ...
            >
              ...
              <div
                class="text-[10px] flex justify-between items-center mt-2 text-[#2D2D30] satoshiM relative"
              >
                <span class="text-darkGold hidden">Edited</span>
                <span class="ms-auto">
                  {{ getTimeDifference(mesg.created_at).time }}
                  {{ getTimeDifference(mesg.created_at).amPm }}
                </span>
                <!-- Emoji -->
            ...
            <div
              v-if="!['image', 'negotiation', 'text'].includes(mesg.type)"
              class="chat-bubble text-black chat_adjustment"
              :class="
                mesg.user_id == store.UserAccount.id
                  ? 'bg-[#F0F2F5]'
            ...
            <div
              ...
              :class="
                mesg.user_id == store.UserAccount.id ? 'chat-start' : 'chat-end'
              "
            >
              <div
                class="chat-bubble bg-brightGold text-black chat_adjustment text-sm rounded-none"
              >
                <span v-if="mesg.negotiation.user_id == store.UserAccount.id"
        ...
        >
          <!-- chat box -->
          <label
            class="w-fullff input px-3 lg:px-4 input-bordered flex items-center gap-1 grow"
          >
            <!-- Select File Button -->
            <button
          ...
          >
            <span
              v-if="defaultLoader"
              class="loading loading-spinner text-white loading-sm"
            />
            <svg
              v-else
              xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="hidden lg:card bg-base-100 w-96 shadow-xl flex-2 h-fit pb-10">
      <div class="card-body">
        ...
        <!-- Services -->
        <div v-if="activeTab == 'job'" class="w-full center">
          <button
            v-if="jobStatus === 'Completed'"
            :disabled="
              (latestOrder.status != 'in review' &&
            ...
            @click="MarkCompleted"
          >
            <span v-if="MCloading" class="loading loading-spinner loading-sm" />
            <span v-else>{{
              latestOrder.status == "completed"
                ? "Completed"
                : "Mark Job as Completed"
        ...
        <!-- Reject offer or report -->
        <div v-if="jobStatus === 'Completed'" class="w-full">
          <!-- Report -->
          <button
            v-if="jobStatus === 'Completed'"
  ...
  <dialog id="my_modal_5" class="modal">
    <div class="modal-box">
      <div class="card-body p-2">
        <!-- Product delivered -->
        <div v-if="!review && activeTab != 'job'" class="">
          ...
          <p class="text-gray-600 mb-6">Seller Proof Of Delivery</p>
        ...
        <!-- if Job is success -->
        <div v-if="review || activeTab == 'job'" class="success">
          <h2
            class="card-title text-black font-bold text-2xl text-center center"
          >
            Success
          </h2>
      ...
      <div class="modal-action">
        <form method="dialog">
          ...
          <button ref="reviewModalBtn" class="btn">Close</button>
        </form>
      </div>
    </div>
  </dialog>
    ...
    <div class="modal-box">
      <div class="card-body p-2">
        ...
        <div class="text-center">
          <p class="mb-3">Valmon Service charge: 7%</p>
          <p class="mb-3">
      ...
      <div class="modal-action">
        <form method="dialog">
          <!-- if there is a button in form, it will close the modal -->
          <button ref="ModalBtn" class="btn">Close</button>
        </form>
      </div>
    </div>
  </dialog>
    ...
    <div class="modal-box">
      <div class="card-body p-2">
        ...
        <p class="text-black mb-3">
          You have marked this product as delivered, please upload proof for
          seller to confirm
        </p>
        <!-- Attachment -->
        <div class="text-[#6E7191]">
          <h1 class="mb-2">Upload Attachment (max 8mb)</h1>
          <div
          ...
          >
            ...
            >
              ...
              <div class="text-sm text-[#5B5B5B] mb-1">
                <span class="text-[#030359]">Select</span> Image you want to
                upload
              </div>
              <div class="text-[#949CA9]">Png and Jpg Allowed</div>
            </div>
            <!-- image uploaded -->
      ...
      >
        <span v-if="orderLoading" class="loading loading-spinner loading-sm" />
        Submit Proof
      </button>
      <!-- Modal trigger for when marked as delivered is successful  -->
      <span ref="modal5" onclick="my_modal_5.showModal()" />
      <div class="modal-action">
        <form method="dialog">
          <!-- if there is a button in form, it will close the modal -->
          <button ref="submitProofBtn" class="btn">Close</button>
        </form>
      </div>
    </div>
  </dialog>
    ...
    <div class="modal-box">
      <div class="card-body p-2">
        ...
        <p class="text-sm text-black">
          Report an issue you might have experienced, and our admin would reach
          out to both parties and resolve the complaint
        </p>
        <label class="form-control w-full">
          <div class="label">
            <span class="label-text">Compliant type</span>
          </div>
          <select v-model="reportData.reason" class="select select-bordered">
      ...
      <div class="modal-action">
        <form method="dialog">
          <!-- if there is a button in form, it will close the modal -->
          <button ref="sendReportBtn" class="btn">Close</button>
        </form>
      </div>
    </div>
  </dialog>
  ...
  <dialog id="my_modal_1" class="modal hidden">
    <div class="modal-box">
      <div class="card-body p-2">
        ...
        <div role="alert" class="alert block bg-[#F0F2F5] mb-3">
          <div class="text-sm text-black mb-2">Payment Amount.</div>
          <div class="text-3xl text-black satoshiB">NGN 195,799</div>
        </div>
        <button
        ...
        >
          <span class="flex flex-row justify-between w-full items-center">
            <img
              ...
              class="w-[40px] h-[40px]"
            />
            <span class="text-[rgba(105, 102, 113, 1)] text-sm font-medium"
              >Axis Bank xxxx68</span
            >
            <span
      ...
      <div class="modal-action">
        <form method="dialog">
          <!-- if there is a button in form, it will close the modal -->
          <button ref="selectCardBtn" class="btn">Close</button>
        </form>
      </div>
    </div>
  </dialog>
  ...
  <!-- Payment -->
  <dialog id="my_modal_2" class="modal hidden">
    <div class="modal-box">
      <div class="card-body p-2">
        <h2 class="card-title text-black font-bold text-2xl text-center center">
          Payment
  ...
  <!-- OTP -->
  <dialog id="my_modal_3" class="modal hidden">
    <div class="modal-box">
      <div class="card-body p-2">
        <h2 class="card-title text-black font-bold text-2xl text-center center">
          OTP
