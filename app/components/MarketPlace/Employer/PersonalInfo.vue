...
<template>
	<div class="">
		<div class="flex justify-center md:justify-between">
			...
			>
				<button v-show="Tab === 'listings'" class="btn btn-neutral">
					Add Listing
				</button>
			</RouterLink>
			<button
	...
	<dialog
		v-show="
			Tab === 'gallery' && store.UserAccount?.account_type === 'worker'
		"
		id="my_modal_1"
		class="modal"
	>
		<div class="modal-box">
			<a href="javascript:void(0)" class="text-black block mb-5">
				<h1 class="satoshiB text-xl text-center mb-5">Upload Image</h1>
				<div
				...
				>
					...
					<div v-if="!draggedFile" class="text-[#949CA9]">
						Png and Jpg Allowed
					</div>
				</div>
			</a>
			...
			<div class="modal-action">
				<form method="dialog">
					...
					<button ref="addGalleryModalBtn" class="btn">Close</button>
				</form>
			</div>
		</div>
	</dialog>
	...
	<dialog id="my_modal_3" class="modal">
		<div class="modal-box">
			...
			>
				...
				<span v-show="edit" class="text-red-600"> Edit Gig</span>
			</h3>
			...
			<label class="text-base text-[#6E7191]" for="">Description</label>
			<textarea
			...
			<div class="modal-action h-0">
				<form method="dialog">
					<!-- if there is a button in form, it will close the modal -->
					<button ref="gigsModal" class="btn">Close</button>
				</form>
			</div>
		</div>
	</dialog>
