...
const address = {
	addresses: [...(store.UserAccount?.profile?.addresses || [])],
};
...
const fetchCountries = async () => {
	...
	if (status.value === "success") CountriesList.value = data.value.data;
...
<template>
	...
	>
		...
		<div>
			<div class="avatar">
				<div class="w-24 rounded-full">
					<img :src="store?.UserAccount?.profile_pic" />
				</div>
				<a
		...
		<div class="flex-1">
			<div class="mb-6">
				<h3
					class="text-[#24242] font-semibold text-lg satoshiM flex items-center gap-2 mb-3"
				>
					<span>
						{{ store.UserAccount.first_name }}
						{{ store.UserAccount.last_name }}</span
					>
				</h3>
				<div
					class="flex gap-2 items-center text-[#62646A] text-xs w-1/2 mb-1"
				>
					<img :src="location" alt="Location icon" class="h-5" />
					<span class="font-medium text-[rgba(0,0,0,1)]"
						>{{ userLocation.city }},
						{{ userLocation.country }}</span
					>
					<a
						href="javascript:void(0);"
				...
				<div class="flex items-center gap-2 mb-1">
					...
					<span class="text-xs font-bold">{{
						store.UserAccount.rating
					}}</span>
					<span class="text-black text-xs"
						>({{ store.UserAccount.ratings_count }} Ratings)</span
					>
				</div>
				<div
					...
					class="flex gap-2 items-center text-[#62646A] text-xs w-1/2 mb-1"
				>
					<img :src="copysuccess" alt="Location icon" class="h-5" />
					<span class="font-medium text-[rgba(0,0,0,1)]"
						>0 jobs Completed</span
					>
				</div>
			</div>
			<!-- Inbox response -->
			...
			>
				...
				>
					...
					<span class="font-bold satoshiM"
						>{{ store.UserAccount.inbox_response_time }} Mins</span
					>
				</div>
				<div
				...
				>
					...
					<span class="font-bold satoshiM"
						>{{ store.UserAccount.inbox_response_rate }}%</span
					>
				</div>
			</div>
			<div class="alert block mb-6">
				<h3
					class="font-bold mb-2 flex items-center gap-2 justify-between"
				>
	...
	<!-- Add Proifle -->
	<dialog id="my_modal_122" class="modal">
		<div class="modal-box">
			...
			<div class="modal-action">
				<form method="dialog">
					...
					<button ref="uploadBtn" class="btn">Close</button>
				</form>
			</div>
		</div>
	</dialog>
	...
	<dialog id="my_modal_10" class="modal">
		<div class="modal-box">
			<h3 class="text-lg font-bold mb-4">Update Bio</h3>

			<label class="text-base text-[#6E7191] mb-3" for=""
				>Description</label
			>
			<textarea
				v-model="userBio.bio"
			...
			<div class="modal-action">
				<form method="dialog">
					<!-- if there is a button in form, it will close the modal -->
					<button class="btn" ref="updateBioBtn">Close</button>
				</form>
			</div>
		</div>
	</dialog>
	...
	<dialog id="my_modal_3" class="modal">
		<div class="modal-box">
			<h3
				class="text-2xl font-bold center mb-4 text-[rgba(30, 30, 30, 1)]"
			>
				Add Address
			</h3>
			...
			<div class="w-full">
				...
				<label class="form-control w-full mb-4">
					<div class="label">
						<span class="label-text-alt text-base text-[#6E7191]"
							>Country</span
						>
					</div>
					<select
				...
				<div class="modal-action h-0">
					<form method="dialog">
						<!-- if there is a button in form, it will close the modal -->
						<button class="btn" ref="addressBtn">Close</button>
					</form>
				</div>
			</div>
		</div>
