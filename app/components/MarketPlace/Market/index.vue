...
const fetchMFLByCategory = async (category) => {
	...
	const { status, data, error } = await getAppListing(
		{ category },
		!!getAuth()
	);
...
const fetchMFL = async () => {
	MFLoader.value = true;
	const { status, data, error } = await getPublicFeaturedListings();
	if (status.value === "success") {
	...
	<div class="px-4 pb-14 pt-0 md:px-20 bg-[#F0F2F5]">
		<div class="w-full mx-auto relative">
			...
			<nuxt-link to="/addlisting">
				<BaseAddButton title="Add Listing" class="absolute right-0" />
			</nuxt-link>
		</div>
	</div>
</template>
