...
const finalStepSubmissionProcess = ref({});
...
const addGalleryStep = () => {
  const formData = new FormData();
		BasicDetails.value.images.forEach((image) => {
...
const nextProcess = (step) => {
	...
	if (step === 2) {
		// Gallery
    finalStepSubmissionProcess.value[step] = addGalleryStep;
    Next("next");
	}
	if (step === 3) {
		// Gig
    loading.value = true;
    Object.values(finalStepSubmissionProcess.value).forEach((func) => {
      func();
    });
		// Next("next");
	}
...
<template>
	<div class="">
		<div ref="setup" onclick="my_modal_2.showModal()" />
